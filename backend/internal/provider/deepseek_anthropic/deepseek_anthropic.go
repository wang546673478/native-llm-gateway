...
func New(cfg provider.ProviderConfig) (provider.Provider, error) {
	...
	if len(cfg.APIKeys) == 0 {
		return nil, fmt.Errorf("deepseek-anthropic requires at least one API key")
	}
	pool, ok := cfg.Pool.(*keypool.Pool)
	if !ok || pool == nil {
		return nil, fmt.Errorf("deepseek-anthropic requires a non-nil keypool.Pool (got %T)", cfg.Pool)
	}
	return &Provider{
